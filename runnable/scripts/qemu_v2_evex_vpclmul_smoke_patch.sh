#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vpclmullqlqdq smoke patch experiment.
#
...
# vmovdqu64, vpshufb, vpaddd, and vpternlogq so the aggregate probe can
# advance past vpclmullqlqdq.
set -euo pipefail
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VPCLMUL_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vpclmul-smoke}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vpclmul_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vpclmul-smoke
...
refresh_paths() {
  ...
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpclmul-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vpclmul-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vpclmul-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpclmul-smoke.patch"
...
+}
+
+static bool rr_try_evex_vpclmul_smoke(DisasContext *s, CPUX86State *env)
+{
+#ifdef TARGET_X86_64
...
+    if (rr_try_evex_vpclmul_smoke(s, env)) {
+        return;
+    }
+
  next_byte:;
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vpclmul-smoke-patched" ]]; then
  ...
  require_tool patch
  log "Applying EVEX vpclmul smoke patch"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vpclmul-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vpclmul-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vpclmul-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
EOF
  ...
  cat >"$PROBE_DIR/vpclmul-semantic.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
build_probe() {
  ...
  grep -F "62 f3 55 48 44 f4 00" "$objdump_file" >/dev/null \
    || die "$name objdump is missing exact vpclmullqlqdq bytes"
  grep -F "vpclmullqlqdq zmm6,zmm5,zmm4" "$objdump_file" >/dev/null \
    || die "$name objdump is missing expected vpclmullqlqdq mnemonic"

  if [[ "$name" == "vpclmul-chain" ]]; then
...
run_one_probe() {
  ...
  if [[ -f "$qemu_log" ]]; then
    ...
    vpclmul_hits="$({ grep -Ehi 'vpclmullqlqdq zmm6,zmm5,zmm4|pclmulqdq' "$qemu_log" || true; } | wc -l)"
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|vpternlogq|vpclmul|pclmul|xor_vec|xor_i64|call.*helper|qemu_(ld|st)[0-9]*_i128' "$qemu_log" | head -80 || true
  return 0
}
...
build_aggregate_probe() {
  ...
  grep -F "62 f3 dd 48 25 eb 96" "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected vpternlogq bytes"
...
run_aggregate_probe() {
  ...
  local result="FAIL_BEFORE_OR_AT_VPCLMUL"
  local next_line
  ...
  if [[ "$run_rc" -eq 0 ]]; then
    result="PASS_ALL"
       ...
       (( 16#$failure_pc_hex > 16#401033 )); then
    result="EXPECTED_FAIL_AFTER_VPCLMUL"
  fi
...
aggregate_vpclmul_hits:       $vpclmul_hits
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|vpternlogq|vpclmul|pclmul|xor_vec|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -120 || true

  case "$result" in
    PASS_ALL|EXPECTED_FAIL_AFTER_VPCLMUL)
      return 0
      ;;
    *)
...
run_probes() {
  ...
  for name in vpclmul-single vpclmul-chain vpclmul-semantic; do
    build_probe "$name"
