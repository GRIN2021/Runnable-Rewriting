#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vmovdqu64 memory smoke patch experiment.
#
# All QEMU source/build/probe outputs stay under /tmp.  The patch is an
# exact-byte experiment for these validated probe byte strings:
#   62 f1 fd 48 ef c0                vpxorq zmm0,zmm0,zmm0
#   62 f1 fd 48 6f c8                vmovdqa64 zmm1,zmm0
#   62 f1 fe 48 7f 0d f6 0f 00 00    vmovdqu64 [rip+scratch],zmm1
#   62 f1 fe 48 6f 15 f6 0f 00 00    vmovdqu64 zmm2,[rip+scratch]
#   62 f1 fe 48 7f 0d ea 0f 00 00    vmovdqu64 [rip+scratch],zmm1
#   62 f1 fe 48 6f 15 e0 0f 00 00    vmovdqu64 zmm2,[rip+scratch]
#
# It builds qemu-x86_64 and runs minimal store/load smoke probes generated
# under /tmp.  A short chain probe reuses the aggregate-probe store/load bytes.
set -euo pipefail
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VMOVDQU64_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vmovdqu64-smoke}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vmovdqu64_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vmovdqu64-smoke
...
refresh_paths() {
  ...
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vmovdqu64-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vmovdqu64-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vmovdqu64-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vmovdqu64-smoke.patch"
...
+}
+
+static bool rr_try_evex_vmovdqu64_smoke(DisasContext *s, CPUX86State *env)
+{
+#ifdef TARGET_X86_64
...
+    if (rr_try_evex_vmovdqu64_smoke(s, env)) {
+        return;
+    }
+
  next_byte:;
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vmovdqu64-smoke-patched" ]]; then
  ...
  require_tool patch
  log "Applying EVEX vmovdqu64 smoke patch"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vmovdqu64-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vmovdqu64-store.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vmovdqu64-load.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
EOF
  ...
  cat >"$PROBE_DIR/vmovdqu64-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
build_probe() {
  ...
  case "$name" in
    vmovdqu64-store)
      grep -F "62 f1 fe 48 7f 0d f6" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected EVEX/opcode bytes"
      grep -F "vmovdqu64 ZMMWORD PTR [rip+0xff6],zmm1" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected mnemonic"
      grep -F "0f 00 00" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected disp32 tail"
      ;;
    vmovdqu64-load)
      grep -F "62 f1 fe 48 6f 15 f6" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected EVEX/opcode bytes"
      grep -F "vmovdqu64 zmm2,ZMMWORD PTR [rip+0xff6]" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected mnemonic"
      grep -F "0f 00 00" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected disp32 tail"
      ;;
    vmovdqu64-chain)
      grep -F "62 f1 fe 48 7f 0d ea" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected store EVEX/opcode bytes"
      grep -F "62 f1 fe 48 6f 15 e0" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected load EVEX/opcode bytes"
      grep -F "0f 00 00" "$objdump_file" >/dev/null \
        || die "$name objdump is missing expected disp32 tail"
      ;;
    *)
      die "unknown probe name: $name"
      ;;
  esac
}
...
EOF
  ...
  if [[ "$result" != "PASS" ]]; then
    ...
    sed -n '1,160p' "$qemu_log" || true
    return 1
  fi

  echo "---- $name qemu memory ops ----"
  grep -En 'qemu_(ld|st)[0-9]*_i128|qemu_(ld|st)[0-9]*_i64' "$qemu_log" | head -20 || true
  return 0
}
...
run_probes() {
  ...
  for name in vmovdqu64-store vmovdqu64-load vmovdqu64-chain; do
    build_probe "$name"
