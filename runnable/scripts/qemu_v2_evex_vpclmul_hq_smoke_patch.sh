#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vpclmullqhqdq smoke patch experiment.
#
...
#
# It preserves the previous exact-byte smoke path for vpxorq, vmovdqa64,
# vmovdqu64, vpshufb, vpaddd, vpternlogq, and vpclmullqlqdq so the aggregate
# probe can advance past vpclmullqhqdq.
set -euo pipefail
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VPCLMUL_HQ_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vpclmul-hq-smoke}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vpclmul_hq_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vpclmul-hq-smoke
...
refresh_paths() {
  ...
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpclmul-hq-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vpclmul-hq-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vpclmul-hq-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpclmul-hq-smoke.patch"
...
+}
+
+static bool rr_try_evex_vpclmul_hq_smoke(DisasContext *s, CPUX86State *env)
+{
+#ifdef TARGET_X86_64
...
+    if (rr_try_evex_vpclmul_hq_smoke(s, env)) {
+        return;
+    }
+
  next_byte:;
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vpclmul-hq-smoke-patched" ]]; then
  ...
  require_tool patch
  log "Applying EVEX vpclmul hq smoke patch"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vpclmul-hq-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vpclmul-hq-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vpclmul-hq-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
EOF
  ...
  cat >"$PROBE_DIR/vpclmul-hq-semantic.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
build_probe() {
  ...
  grep -F "62 f3 55 48 44 fc 10" "$objdump_file" >/dev/null \
    || die "$name objdump is missing exact vpclmullqhqdq bytes"
  grep -F "vpclmullqhqdq zmm7,zmm5,zmm4" "$objdump_file" >/dev/null \
    || die "$name objdump is missing expected vpclmullqhqdq mnemonic"

  if [[ "$name" == "vpclmul-hq-chain" ]]; then
    ...
    grep -F "62 f2 6d 48 00 da" "$objdump_file" >/dev/null \
      || die "$name objdump is missing vpshufb bytes"
    grep -F "62 f1 65 48 fe e2" "$objdump_file" >/dev/null \
      || die "$name objdump is missing vpaddd bytes"
    grep -F "62 f3 dd 48 25 eb 96" "$objdump_file" >/dev/null \
      || die "$name objdump is missing vpternlogq bytes"
...
run_one_probe() {
  ...
  local vpaddd_hits=0
  local vpternlogq_hits=0
  local vpclmul_lqlq_hits=0
  local vpclmul_lqhq_hits=0
  ...
  if [[ -f "$qemu_log" ]]; then
    ...
    vpaddd_hits="$({ grep -Ehi 'add_vec v256,e32' "$qemu_log" || true; } | wc -l)"
    vpternlogq_hits="$({ grep -Ehi 'vpternlogq zmm5,zmm4,zmm3,0x96' "$qemu_log" || true; } | wc -l)"
    vpclmul_lqlq_hits="$({ grep -Ehi 'vpclmullqlqdq zmm6,zmm5,zmm4' "$qemu_log" || true; } | wc -l)"
    vpclmul_lqhq_hits="$({ grep -Ehi 'vpclmullqhqdq zmm7,zmm5,zmm4' "$qemu_log" || true; } | wc -l)"
    pclmul_helper_hits="$({ grep -Ehi 'pclmulqdq' "$qemu_log" || true; } | wc -l)"
    xor_hits="$({ grep -Ehi 'xor_vec|xor_i64|xor_i32' "$qemu_log" || true; } | wc -l)"
...
vpshufb_hits:          $vpshufb_hits
vpaddd_hits:           $vpaddd_hits
vpternlogq_hits:       $vpternlogq_hits
vpclmul_lqlq_hits:     $vpclmul_lqlq_hits
vpclmul_lqhq_hits:     $vpclmul_lqhq_hits
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|vpternlogq|vpclmul|pclmul|xor_vec|xor_i64|call.*helper|qemu_(ld|st)[0-9]*_i128' "$qemu_log" | head -120 || true
  return 0
}
...
build_aggregate_probe() {
  ...
  grep -F "62 f3 55 48 44 f4 00" "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected vpclmullqlqdq bytes"
...
run_aggregate_probe() {
  ...
  local result="FAIL_BEFORE_OR_AT_VPCLMUL_HQ"
  local next_line
  ...
  if [[ "$run_rc" -eq 0 ]]; then
    result="PASS_ALL"
       ...
       (( 16#$failure_pc_hex > 16#40103a )); then
    result="EXPECTED_FAIL_AFTER_VPCLMUL_HQ"
  fi
...
aggregate_vpternlogq_hits:    $vpternlogq_hits
aggregate_vpclmul_lqlq_hits:  $vpclmul_lqlq_hits
aggregate_vpclmul_lqhq_hits:  $vpclmul_lqhq_hits
aggregate_pclmul_helper_hits: $pclmul_helper_hits
aggregate_xor_hits:           $xor_hits
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|vpternlogq|vpclmul|pclmul|xor_vec|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -160 || true

  case "$result" in
    PASS_ALL|EXPECTED_FAIL_AFTER_VPCLMUL_HQ)
      return 0
      ;;
    *)
...
run_probes() {
  ...
  for name in vpclmul-hq-single vpclmul-hq-chain vpclmul-hq-semantic; do
    build_probe "$name"
