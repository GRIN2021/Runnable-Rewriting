#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vaesenc smoke patch experiment.
#
# All QEMU source/build/probe outputs stay under /tmp. This script reuses the
# existing VPCLMUL HH smoke patch as the carried-forward aggregate prefix, then
...
HH_SCRIPT="$SCRIPT_DIR/qemu_v2_evex_vpclmul_hh_smoke_patch.sh"
AGGREGATE_SRC="$REPO_ROOT/test/qemu-v2-probes/avx512-evex.S"

SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VAESENC_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vaesenc-smoke}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vaesenc_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vaesenc-smoke
...
refresh_paths() {
  HH_ROOT="$SCRATCH_ROOT/hh-base"
  HH_PATCHED_SRC="$HH_ROOT/qemu-${QEMU_VERSION}-evex-vpclmul-hh-smoke-src"
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vaesenc-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vaesenc-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vaesenc-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vaesenc-overlay.patch"
...
@@ -2642,6 +2642,19 @@ static void rr_evex_pclmul_xmm_lane(intptr_t dofs, intptr_t vofs,
...
@@ -2765,6 +2778,26 @@ static void rr_evex_pclmul_zmm9_zmm5_zmm4_hqhq_512(void)
...
+
 ...
 {
 #ifdef TARGET_X86_64
@@ -2813,6 +2846,9 @@ static bool rr_try_evex_vpclmul_hqlq_smoke(DisasContext *s, CPUX86State *env)
...
@@ -2895,6 +2931,13 @@ static bool rr_try_evex_vpclmul_hqlq_smoke(DisasContext *s, CPUX86State *env)
...
prepare_hh_base() {
  [[ -f "$HH_SCRIPT" ]] || die "missing HH base smoke script: $HH_SCRIPT"

  local args=(
    --scratch-root "$HH_ROOT"
    --jobs "$JOBS"
    --skip-aggregate
  )
  ...
  log "Preparing carried-forward HH smoke source under $HH_ROOT"
  bash "$HH_SCRIPT" "${args[@]}"
  [[ -d "$HH_PATCHED_SRC" ]] || die "missing HH patched source: $HH_PATCHED_SRC"
}

prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vaesenc-smoke-patched" ]]; then
  ...
  log "Copying HH source tree for VAESENC overlay"
  cp -a "$HH_PATCHED_SRC" "$PATCHED_SRC"

  require_tool patch
  log "Applying EVEX vaesenc exact-byte overlay"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vaesenc-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vaesenc-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vaesenc-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
EOF
  ...
  cat >"$PROBE_DIR/vaesenc-semantic.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
build_probe() {
  ...
  grep -F "62 52 35 48 dc d0" "$objdump_file" >/dev/null \
    || die "$name objdump is missing exact vaesenc bytes"
  grep -F "vaesenc zmm10,zmm9,zmm8" "$objdump_file" >/dev/null \
    || die "$name objdump is missing expected vaesenc mnemonic"

  if [[ "$name" == "vaesenc-chain" ]]; then
...
run_one_probe() {
  ...
  if [[ "$run_rc" -eq 0 && "$exception_hits" -eq 0 && "$aesenc_helper_hits" -ge 4 ]]; then
    result="PASS"
  fi
...
EOF
  ...
  grep -En 'vaesenc|aesenc|vpclmul|pclmul|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -180 || true
  return 0
}
...
build_aggregate_probe() {
  ...
  grep -F "vaesenclast zmm11,zmm10,zmm7" "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected post-VAESENC vaesenclast instruction"
}
...
run_aggregate_probe() {
  ...
  local result="FAIL_BEFORE_OR_AT_VAESENC"
  local next_line
  ...
  if [[ "$run_rc" -eq 0 ]]; then
    result="PASS_ALL"
       ...
       (( 16#$failure_pc_hex > 16#40104f )) &&
       [[ "$pclmul_helper_hits" -ge 16 ]] &&
       [[ "$aesenc_helper_hits" -ge 4 ]]; then
    result="EXPECTED_FAIL_AFTER_VAESENC"
  fi
...
EOF
  ...
  grep -En 'vpclmul|pclmul|vaes|aesenc|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -220 || true

  case "$result" in
    PASS_ALL|EXPECTED_FAIL_AFTER_VAESENC)
      return 0
      ;;
    *)
...
run_probes() {
  ...
  for name in vaesenc-single vaesenc-chain vaesenc-semantic; do
    build_probe "$name"
