#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vextracti32x4 smoke patch experiment.
#
# All QEMU source/build/probe outputs stay under /tmp. This script expects a
# verified base source tree that already includes the vpsrldq smoke patch, then
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VEXTRACTI32X4_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vextracti32x4-smoke}"
BASE_SRC="${RUNNABLE_QEMU_V2_EVEX_VEXTRACTI32X4_BASE_SRC:-}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vextracti32x4_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vextracti32x4-smoke
  --base-src DIR         Verified QEMU source tree that already includes
                         the vpsrldq smoke overlay.
...
refresh_paths() {
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vextracti32x4-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vextracti32x4-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vextracti32x4-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vextracti32x4-smoke.patch"
...
resolve_base_src() {
  [[ -n "$BASE_SRC" ]] || \
    die "no verified base source tree available; pass --base-src with a vpsrldq smoke tree or use full-harness-16 patched source"
...
ensure_patch_file() {
  ...
  modified_file="$(mktemp "$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vextracti32x4.XXXXXX")"
...
anchor = """static bool rr_try_evex_vpsrldq_smoke(DisasContext *s, CPUX86State *env)
{
#ifdef TARGET_X86_64
...
insertion = """static bool rr_try_evex_vpsrldq_smoke(DisasContext *s, CPUX86State *env)
{
#ifdef TARGET_X86_64
    static const uint8_t vpsrldq_zmm14_zmm13_0x4[] = {
        0x62, 0xd1, 0x0d, 0x48, 0x73, 0xdd, 0x04
...
if "static bool rr_try_evex_vextracti32x4_smoke(DisasContext *s, CPUX86State *env)" in src:
    raise SystemExit("vextracti32x4 overlay already present in source")
if anchor not in src:
    raise SystemExit("failed to find vpsrldq anchor for vextracti32x4 overlay")
src = src.replace(anchor, insertion, 1)

call_marker = """    if (rr_try_evex_vpsrldq_smoke(s, env)) {
        return;
    }"""
call_insert = """    if (rr_try_evex_vpsrldq_smoke(s, env)) {
        return;
    }
...
if call_marker not in src:
    raise SystemExit("failed to find vpsrldq dispatch hook")
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vextracti32x4-smoke-patched" ]]; then
  ...
  resolve_base_src
  log "Copying verified base source tree for vextracti32x4 overlay"
  cp -a "$BASE_SRC" "$PATCHED_SRC"

  require_tool patch
  log "Applying EVEX vextracti32x4 exact-byte overlay"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vextracti32x4-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vextracti32x4-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vextracti32x4-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
run_probe() {
  ...
  insn_hits="$(count_hits '62537d4839f701|62 53 7d 48 39 f7 01|vextracti32x4 xmm15,zmm14,0x1|vextracti32x4' "$log_file" "$out")"
  copy_hits="$(count_hits 'st_vec v256,e8,tmp10,env,\$0x720|st_vec v256,e8,tmp10,env,\$0x740|st_i64 .*env,\$0x720|st_i64 .*env,\$0x728' "$log_file")"

  printf '%s: %s rc=%s vextracti32x4_hits=%s copy_hits=%s exception_hits=%s\n' \
    "$name" \
...
build_aggregate_probe() {
  ...
  grep -F '62 53 7d 48 39 f7 01' "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected vextracti32x4 bytes"
  grep -F 'vextracti32x4 xmm15,zmm14,0x1' "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected vextracti32x4 mnemonic"
}

run_aggregate_probe() {
  local out="$OUT_DIR/aggregate.max.out"
  local log_file="$OUT_DIR/aggregate.max.qemu.log"
  local rc exception_hits observed_pc next_line vextracti32x4_hits prefix_hits
  ...
  vextracti32x4_hits="$(count_hits '62537d4839f701|62 53 7d 48 39 f7 01|vextracti32x4 xmm15,zmm14,0x1|vextracti32x4' "$log_file" "$out")"
  prefix_hits="$(count_hits 'psrldq_xmm|vpsrldq zmm14,zmm13,0x4|rip,\$0x40106c|0x000000000040106c' "$log_file" "$out")"
  next_line="$(grep -E -i -m1 '^ *40107a:' "$OUT_DIR/aggregate.objdump.txt" | sed 's/[[:space:]][[:space:]]*/ /g' || true)"

  printf 'aggregate: %s expected_next=vextracti64x4 expected_pc=0x40107a observed_pc=%s rc=%s vextracti32x4_hits=%s exception_hits=%s\n' \
    "$(if [[ "$rc" != "0" && "$exception_hits" -gt 0 && "$observed_pc" = "0x40107a" && ( "$vextracti32x4_hits" -gt 0 || "$prefix_hits" -gt 0 ) ]]; then echo PASS; else echo FAIL; fi)" \
    "${observed_pc:-unknown}" "$rc" "$vextracti32x4_hits" "$exception_hits"
  printf 'aggregate_next: %s\n' "${next_line:-unknown}"

  [[ "$rc" != "0" && "$exception_hits" -gt 0 && "$observed_pc" = "0x40107a" && ( "$vextracti32x4_hits" -gt 0 || "$prefix_hits" -gt 0 ) ]]
}
...
main() {
  ...
  build_probe "vextracti32x4-single" "62537d4839f701" "vextracti32x4 xmm15,zmm14,0x1"
  build_probe "vextracti32x4-chain" "62537d4839f701" "vextracti32x4 xmm15,zmm14,0x1"

  run_probe "vextracti32x4-single"
  run_probe "vextracti32x4-chain"
  run_aggregate_probe
}
