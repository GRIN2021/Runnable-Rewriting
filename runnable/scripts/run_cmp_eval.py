...
STATIC_FALLBACK_PLACEHOLDERS = {"bb", "marker"}
    ...
    for start, end in merge_static_fallback_ranges(ranges):
        ...
        while obj_index < obj_count:
            ...
            if addr in ll_instructions:
                if ll_instructions[addr] in STATIC_FALLBACK_PLACEHOLDERS:
    ...
    if static_fallback.get("enabled"):
        lines.extend(
            [
                ...
                "static_fallback_replaced_placeholders="
                f"{static_fallback.get('replaced_placeholders', 0)}",
            ]
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main():
        ...
        static_fallback.update(
            {
                ...
                "replaced_placeholders": stats["replaced_placeholders"],
                "ranges": [
                    {
                        "name": item["name"],
    ...
    if payload["static_fallback"]["enabled"]:
        ...
        print(
            "static_fallback_replaced_placeholders="
            f"{payload['static_fallback']['replaced_placeholders']}"
        )
