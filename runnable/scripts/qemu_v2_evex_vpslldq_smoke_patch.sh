#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vpslldq smoke patch experiment.
#
# All QEMU source/build/probe outputs stay under /tmp. This script expects a
# verified base source tree that already includes the vbroadcastf64x2 smoke
# patch, then applies a small exact-byte overlay for:
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VPSLLDQ_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vpslldq-smoke}"
BASE_SRC="${RUNNABLE_QEMU_V2_EVEX_VPSLLDQ_BASE_SRC:-}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vpslldq_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vpslldq-smoke
  --base-src DIR         Verified QEMU source tree that already includes
                         the vbroadcastf64x2 smoke overlay.
...
refresh_paths() {
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpslldq-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vpslldq-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vpslldq-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpslldq-smoke.patch"
...
resolve_base_src() {
  [[ -n "$BASE_SRC" ]] || \
    die "no verified base source tree available; pass --base-src with a vbroadcastf64x2 smoke tree or run patch-series 0014 PASS first"
...
ensure_patch_file() {
  ...
  modified_file="$(mktemp "$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpslldq.XXXXXX")"
...
src = orig
vpslldq_block = """static TCGv_ptr rr_evex_make_imm8u_xmm_vec(uint8_t imm)
{
...
marker = "static bool rr_try_evex_vbroadcastf64x2_smoke(DisasContext *s, CPUX86State *env)"
if "static bool rr_try_evex_vpslldq_smoke(DisasContext *s, CPUX86State *env)" in src:
    raise SystemExit("vpslldq overlay already present in source")
if marker not in src:
    raise SystemExit("failed to find insertion point for vpslldq overlay")
src = src.replace(marker, vpslldq_block + marker, 1)

call_marker = "    if (rr_try_evex_vbroadcastf64x2_smoke(s, env)) {\n        return;\n    }"
call_insert = """    if (rr_try_evex_vbroadcastf64x2_smoke(s, env)) {
        return;
    }
...
if call_marker not in src:
    raise SystemExit("failed to find vbroadcast dispatch hook")
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vpslldq-smoke-patched" ]]; then
  ...
  resolve_base_src
  log "Copying verified base source tree for VPSLLDQ overlay"
  cp -a "$BASE_SRC" "$PATCHED_SRC"

  require_tool patch
  log "Applying EVEX vpslldq exact-byte overlay"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vpslldq-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vpslldq-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vpslldq-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
build_probe() {
  ...
  grep -F "$expected_objdump" "$objdump_file" >/dev/null \
    || die "$name objdump is missing expected vpslldq/vbroadcast mnemonic"
}
...
run_one_probe() {
  ...
  if [[ "$run_rc" -eq 0 && "$exception_hits" -eq 0 && "$pslldq_helper_hits" -ge 4 ]]; then
    result="PASS"
  fi
...
EOF
  ...
  grep -En 'vbroadcast|pslldq_xmm|qemu_ld[2]?_i128|st_i(64|128)|vaes|aesenc|aesenclast|vpclmul|pclmul|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -240 || true
  return 0
}
...
build_aggregate_probe() {
  ...
  require_binary_bytes "$AGGREGATE_BIN" "62d1154873fc04" "aggregate"
  grep -F "62 d1 15 48 73 fc 04" "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected vpslldq bytes"
  ...
  grep -F "vpsrldq zmm14,zmm13,0x4" "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected post-vpslldq vpsrldq instruction"
}
...
run_aggregate_probe() {
  ...
  local pclmul_helper_hits=0
  local failure_pc_hex=""
  local result="FAIL_BEFORE_OR_AT_VPSLLDQ"
  local next_line
...
EOF
  ...
  grep -En 'vpclmul|pclmul|vaes|aesenc|aesenclast|vbroadcast|pslldq_xmm|qemu_ld[2]?_i128|st_i(64|128)|vpslldq|vpsrldq|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -300 || true
...
run_probes() {
  ...
  build_probe "vpslldq-single" "62d1154873fc04" \
    "vpslldq zmm13,zmm12,0x4"
  build_probe "vpslldq-chain" "62d1154873fc04" \
    "vpslldq zmm13,zmm12,0x4"

  for name in vpslldq-single vpslldq-chain vpslldq-semantic; do
    case "$name" in
      vpslldq-semantic)
        cat <<'EOF'
probe:                    vpslldq-semantic
result:                   SKIPPED
skip_reason:              semantic verification would require a zmm13 store hook, which is outside this exact-byte smoke
EOF
        ;;
      *)
