...
def seed_addr_range_flags(
    args: argparse.Namespace,
    *,
    start: int,
    end_exclusive: int,
    shard_start: int,
    shard_end_exclusive: int,
) -> List[str]:
    if has_addr_range_flags(args.coordinator_flag):
        return []
    if args.range_mode == "none":
        return []
    if args.range_mode == "shard":
        start = shard_start
        end_exclusive = shard_end_exclusive
    return [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--range-mode", choices=("seed", "shard", "none"), default="seed")
...
def run_seed(
    *,
    ...
    flags = [
        ...
        *seed_addr_range_flags(
            args,
            start=start,
            end_exclusive=end_exclusive,
            shard_start=shard_start,
            shard_end_exclusive=shard_end_exclusive,
        ),
...
def run_shard(args: argparse.Namespace, shard: Dict[str, object], *, jsonl_lock: threading.Lock) -> Dict[str, object]:
    ...
    shard_start = int(str(shard["start_hex"]), 0)
    shard_end_exclusive = int(str(shard["end_exclusive_hex"]), 0)
