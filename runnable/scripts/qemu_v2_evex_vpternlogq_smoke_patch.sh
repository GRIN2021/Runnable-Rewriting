#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vpternlogq smoke patch experiment.
#
...
# vmovdqu64, vpshufb, and vpaddd so the aggregate probe can advance past
# vpternlogq.
set -euo pipefail
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VPTERNLOGQ_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vpternlogq-smoke}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vpternlogq_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vpternlogq-smoke
...
refresh_paths() {
  ...
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpternlogq-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vpternlogq-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vpternlogq-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpternlogq-smoke.patch"
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vpternlogq-smoke-patched" ]]; then
  ...
  require_tool patch
  log "Applying EVEX vpternlogq smoke patch"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vpternlogq-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vpternlogq-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vpternlogq-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
EOF
  ...
  cat >"$PROBE_DIR/vpternlogq-semantic.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
_start:
    movdqu xmm5, xmmword ptr [rip + src_dest]
    movdqu xmm4, xmmword ptr [rip + src2]
    movdqu xmm3, xmmword ptr [rip + src3]
    .byte 0x62, 0xf3, 0xdd, 0x48, 0x25, 0xeb, 0x96
    movdqu xmmword ptr [rip + out], xmm5
...
src3:
    .quad 0x0101010101010101, 0x1010101010101010
expected:
    .quad 0x1133557799bbddff, 0xccEE88AA44660022
out:
    .zero 16
EOF
}
...
build_probe() {
  ...
  grep -F "62 f3 dd 48 25 eb 96" "$objdump_file" >/dev/null \
    || die "$name objdump is missing exact vpternlogq bytes"
  grep -F "vpternlogq zmm5,zmm4,zmm3,0x96" "$objdump_file" >/dev/null \
    || die "$name objdump is missing expected vpternlogq mnemonic"

  if [[ "$name" == "vpternlogq-chain" ]]; then
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|vpternlogq|xor_vec|xor_i64|call.*helper|qemu_(ld|st)[0-9]*_i128' "$qemu_log" | head -60 || true
  return 0
}
...
build_aggregate_probe() {
  ...
  grep -F "62 f1 65 48 fe e2" "$OUT_DIR/aggregate.objdump.txt" >/dev/null \
    || die "aggregate objdump is missing expected vpaddd bytes"
...
run_aggregate_probe() {
  ...
  local result="FAIL_BEFORE_OR_AT_VPTERNLOGQ"
  local next_line
  ...
  if [[ "$run_rc" -eq 0 ]]; then
    result="PASS_ALL"
  elif [[ -n "$failure_pc_hex" && "$failure_pc_hex" != "40102c" ]]; then
    result="EXPECTED_FAIL_AFTER_VPTERNLOGQ"
  fi
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|vpternlogq|xor_vec|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -100 || true

  case "$result" in
    PASS_ALL|EXPECTED_FAIL_AFTER_VPTERNLOGQ)
      return 0
      ;;
    *)
...
run_probes() {
  ...
  for name in vpternlogq-single vpternlogq-chain vpternlogq-semantic; do
    build_probe "$name"
