...
Options:
  ...
  --download-qemu         For --libtinycode, download and verify QEMU 10.2.3
                          under the build directory when no source tree is
                          supplied.
  ...
  --external-binary PATH  Build a request-aware live-sidecar runtime for a
                          caller-supplied executable. Requires --external-entry.
  --external-entry HEX    Initial guest PC used by the live-sidecar smoke
                          harness for --external-binary.
  --external-label NAME   Label for external walker artifacts.
...
download_qemu_10_2_3_source() {
  local destination_root="$1"
  local download_dir tarball source_dir

  download_dir="$destination_root/qemu-source-download"
  tarball="$download_dir/$QEMU_TARBALL"
  source_dir="$download_dir/qemu-$QEMU_VERSION"
  mkdir -p "$download_dir"

  if [[ ! -f "$tarball" ]]; then
    command -v curl >/dev/null 2>&1 || die "curl is required for --download-qemu"
    echo "downloading QEMU $QEMU_VERSION: $QEMU_URL"
  ...
  is_qemu_10_2_3_tree "$source_dir" || die "downloaded archive did not produce QEMU $QEMU_VERSION source: $source_dir"
  (cd "$source_dir" && pwd -P)
}
...
build_libtinycode_v2() {
  ...
  local external_binary_abs external_run_dir_abs
  local live_script helper_generator compat_header
  ...
  compat_header="$RR_DIR/runnable/include/qemu-v2-compat/ptc.h"
  ...
  [[ -f "$compat_header" ]] || die "QEMU V2 PTC compatibility header not found: $compat_header"

  build_dir_abs="$(resolve_output_dir "$BUILD_DIR")"
  install_dir_abs="$(resolve_output_dir "$INSTALL_DIR")"
  if [[ "$DOWNLOAD_QEMU" -eq 1 ]]; then
    [[ "$QEMU_SRC_SET" -eq 0 ]] || die "--download-qemu and --qemu-src are mutually exclusive"
    qemu_src_abs="$(download_qemu_10_2_3_source "$build_dir_abs")"
  else
    qemu_src_abs="$(require_qemu_10_2_3_tree "$QEMU_SRC")"
  fi
  qemu_version="$(tr -d '[:space:]' < "$qemu_src_abs/VERSION")"
  ...
  if [[ -n "$EXTERNAL_BINARY" ]]; then
    [[ -n "$EXTERNAL_ENTRY" ]] || die "--external-entry is required with --external-binary"
    external_binary_abs="$(resolve_existing_file "$EXTERNAL_BINARY" "external binary")"
    [[ -x "$external_binary_abs" ]] || die "external binary is not executable: $external_binary_abs"
    live_args+=(--external-binary "$external_binary_abs" --external-entry "$EXTERNAL_ENTRY")
  elif [[ -n "$EXTERNAL_ENTRY" ]]; then
    die "--external-entry requires --external-binary"
  fi
  if [[ -n "$EXTERNAL_LABEL" ]]; then
    live_args+=(--external-label "$EXTERNAL_LABEL")
  fi
  if [[ -n "$EXTERNAL_RUN_DIR" ]]; then
    external_run_dir_abs="$(resolve_existing_dir "$EXTERNAL_RUN_DIR" "external run directory")"
    live_args+=(--external-run-dir "$external_run_dir_abs")
  fi
  if [[ -n "$GUEST_BASE" ]]; then
    live_args+=(--guest-base "$GUEST_BASE")
  fi
  ...
  cp "$compat_header" "$install_dir_abs/include/ptc.h"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --replay-summary)
      ...
      ;;
    --external-binary)
      EXTERNAL_BINARY="${2:?missing value for --external-binary}"
      shift 2
      ;;
    --external-entry)
    ...
    --external-label)
      ...
      ;;
    --external-run-dir)
      EXTERNAL_RUN_DIR="${2:?missing value for --external-run-dir}"
      shift 2
      ;;
    --guest-base)
...
if [[ "$MODE" == "libtinycode" ]]; then
  ...
  if [[ "$USE_DOCKER" != "never" ]] && ! is_container; then
    ...
    local_external_binary_abs=""
    local_external_run_dir_abs=""
    DOCKER_ARGS=()
    USER_ARGS=()
    DOCKER_MOUNTS=()
    ...
    if [[ "$DOWNLOAD_QEMU" -eq 1 ]]; then
      DOCKER_ARGS+=(--download-qemu)
    else
      DOCKER_ARGS+=(--qemu-src "$QEMU_SRC_CONTAINER")
    fi
    if [[ -n "$REPLAY_PAYLOAD" ]]; then
    ...
    if [[ -n "$EXTERNAL_BINARY" ]]; then
      [[ -n "$EXTERNAL_ENTRY" ]] || die "--external-entry is required with --external-binary"
      local_external_binary_abs="$(resolve_existing_file "$EXTERNAL_BINARY" "external binary")"
      [[ -x "$local_external_binary_abs" ]] || die "external binary is not executable: $local_external_binary_abs"
      set_container_handoff_file_path EXTERNAL_BINARY_CONTAINER "$local_external_binary_abs" "/tmp/runnable-qemu-v2/external-binary"
      DOCKER_ARGS+=(--external-binary "$EXTERNAL_BINARY_CONTAINER" --external-entry "$EXTERNAL_ENTRY")
    elif [[ -n "$EXTERNAL_ENTRY" ]]; then
      die "--external-entry requires --external-binary"
    fi
    if [[ -n "$EXTERNAL_LABEL" ]]; then
      DOCKER_ARGS+=(--external-label "$EXTERNAL_LABEL")
    fi
    if [[ -n "$EXTERNAL_RUN_DIR" ]]; then
      local_external_run_dir_abs="$(resolve_existing_dir "$EXTERNAL_RUN_DIR" "external run directory")"
      set_container_handoff_dir_path EXTERNAL_RUN_DIR_CONTAINER "$local_external_run_dir_abs" "/tmp/runnable-qemu-v2/external-run-dir" "ro"
      DOCKER_ARGS+=(--external-run-dir "$EXTERNAL_RUN_DIR_CONTAINER")
    fi
    if [[ -n "$GUEST_BASE" ]]; then
      DOCKER_ARGS+=(--guest-base "$GUEST_BASE")
    fi
