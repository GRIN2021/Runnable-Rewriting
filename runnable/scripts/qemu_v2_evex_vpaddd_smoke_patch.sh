#!/usr/bin/env bash
#
# Throwaway QEMU 10.2.3 EVEX vpaddd smoke patch experiment.
#
...
# vmovdqu64, and vpshufb so the aggregate probe can advance past vpaddd.
set -euo pipefail
...
SCRATCH_ROOT="${RUNNABLE_QEMU_V2_EVEX_VPADDD_SMOKE_ROOT:-/tmp/rr-qemu-v2-evex-vpaddd-smoke}"
...
usage() {
  cat <<'EOF'
Usage:
  qemu_v2_evex_vpaddd_smoke_patch.sh [options]

Options:
  --scratch-root DIR     Scratch root under /tmp.
                         Default: /tmp/rr-qemu-v2-evex-vpaddd-smoke
...
refresh_paths() {
  ...
  PATCHED_SRC="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpaddd-smoke-src"
  BUILD_DIR="$SCRATCH_ROOT/build-${QEMU_VERSION}-evex-vpaddd-smoke"
  INSTALL_DIR="$SCRATCH_ROOT/install-${QEMU_VERSION}-evex-vpaddd-smoke"
  PATCH_FILE="$SCRATCH_ROOT/qemu-${QEMU_VERSION}-evex-vpaddd-smoke.patch"
...
prepare_patched_tree() {
  if [[ -d "$PATCHED_SRC" ]]; then
    if [[ -f "$PATCHED_SRC/.rr-evex-vpaddd-smoke-patched" ]]; then
  ...
  require_tool patch
  log "Applying EVEX vpaddd smoke patch"
  (cd "$PATCHED_SRC" && patch -p1 <"$PATCH_FILE")
  touch "$PATCHED_SRC/.rr-evex-vpaddd-smoke-patched"
}
...
write_probe_sources() {
  ...
  cat >"$PROBE_DIR/vpaddd-single.S" <<'EOF'
...
EOF
  ...
  cat >"$PROBE_DIR/vpaddd-chain.S" <<'EOF'
.intel_syntax noprefix
.section .text
.global _start
.type _start, @function
...
build_probe() {
  ...
  grep -F "62 f1 65 48 fe e2" "$objdump_file" >/dev/null \
    || die "$name objdump is missing exact vpaddd bytes"
  grep -F "vpaddd zmm4,zmm3,zmm2" "$objdump_file" >/dev/null \
    || die "$name objdump is missing expected vpaddd mnemonic"

  if [[ "$name" == "vpaddd-chain" ]]; then
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|call.*helper|qemu_(ld|st)[0-9]*_i128' "$qemu_log" | head -40 || true
  return 0
}
...
run_aggregate_probe() {
  ...
  local result="FAIL_BEFORE_OR_AT_VPADDD"
  local next_line
  ...
  if [[ "$run_rc" -eq 0 ]]; then
    result="PASS_ALL"
  elif [[ -n "$failure_pc_hex" && "$failure_pc_hex" != "401026" ]]; then
    result="EXPECTED_FAIL_AFTER_VPADDD"
  fi
...
EOF
  ...
  grep -En 'vpshufb|vpaddd|raise_exception|check_exception|rip,\$0x[0-9a-f]+' "$qemu_log" | head -80 || true

  case "$result" in
    PASS_ALL|EXPECTED_FAIL_AFTER_VPADDD)
      return 0
      ;;
    *)
...
run_probes() {
  ...
  for name in vpaddd-single vpaddd-chain; do
    build_probe "$name"
