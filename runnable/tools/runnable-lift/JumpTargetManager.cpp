...
#include "runnable/ADT/Queue.h"
#include "runnable/BasicAnalyses/GeneratedCodeBasicInfo.h"
...
bool TranslateDirectBranchesPass::pinJTs(Function &F) {
  ...
  for (const auto &Jump : SET->jumps()) {
    ...
    runnable_assert(CallExitTB != nullptr);
    runnable_assert(PCWrite->getParent()->getParent() == &F);
    runnable_assert(JTM->isPCReg(PCWrite->getPointerOperand()));
    runnable_assert(Destinations.size() != 0);
  ...
  while (ExitTBIt != ExitTB->use_end()) {
    ...
    if (auto Call = dyn_cast<CallInst>(ExitTBUse.getUser())) {
      if (Call->getCalledFunction() == ExitTB) {
        ...
        runnable_unreachable("Unexpected instruction using the PC");
      }
    } else {
      runnable_unreachable("Unhandled usage of the PC");
    }
  }
...
uint64_t TranslateDirectBranchesPass::getNextPC(Instruction *TheInstruction) {
  ...
  runnable_unreachable("Can't find the PC marker");
}
...
JumpTargetManager::JumpTargetManager(Function *TheFunction,
  ...
  if(Binary.rodataStartAddr){
    ...
    runnable_assert(ro_StartAddr<=ro_EndAddr);
  }
    ...
    VisitAction visit(BasicBlockRange Range) {
      for (Instruction &I : Range) {
        if (auto *Call = dyn_cast<CallInst>(&I)) {
          runnable_assert(!(Call->getCalledFunction()->getName() == "newpc"));
          if (Call->getCalledFunction() == ExitTB) {
  ...
  while (!WorkList.empty()) {
    ...
    if (NewPCCall == nullptr) {
      ...
      for (BasicBlock *Predecessor : predecessors(BB)) {
        ...
        runnable_assert(!(NewPCCall == nullptr && Predecessor == Dispatcher));
...
void JumpTargetManager::handleSumJump(Instruction *SumJump) {
  ...
  runnable_assert(NextPC != 0);
  BasicBlock *BB = registerJT(NextPC, JTReason::SumJump);
  runnable_assert(BB && !BB->empty());
...
void JumpTargetManager::translateIndirectJumps() {
  ...
  while (I != ExitTB->use_end()) {
    ...
    if (auto *Call = dyn_cast<CallInst>(ExitTBUse.getUser())) {
      if (Call->getCalledFunction() == ExitTB) {
        ...
        if (PCWrite != nullptr) {
        //   runnable_assert(!isa<ConstantInt>(PCWrite->getValueOperand()),
  ...
  runnable_assert(ExitTB->use_empty());
...
BasicBlock *JumpTargetManager::getBlockAt(uint64_t PC) {
  ...
  runnable_assert(TargetIt != JumpTargets.end());
...
void JumpTargetManager::purgeTranslation(BasicBlock *Start) {
  ...
  for (BasicBlock *BB : Visited) {
    ...
    while (pred_begin(BB) != pred_end(BB)) {
      ...
      runnable_assert(pred_empty(Predecessor));
      Predecessor->eraseFromParent();
    }

    runnable_assert(BB->use_empty());
    BB->eraseFromParent();
  }
}
...
JumpTargetManager::registerJT(uint64_t PC, JTReason::Values Reason) {
  ...
  if (InstrIt != OriginalInstructionAddresses.end()) {
    ...
    if (isFirst(I)) {
      ...
      runnable_assert(I != nullptr && I->getIterator() != ContainingBlock->end());
...
void JumpTargetManager::setCFGForm(CFGForm::Values NewForm) {
  runnable_assert(CurrentCFGForm != NewForm);
  runnable_assert(NewForm != CFGForm::UnknownFormCFG);
  ...
  default:
    runnable_abort("Not implemented yet");
  }

  QuickMetadata QMD(Context);
  AnyPC->getTerminator()->setMetadata("runnable.block.type",
                                      QMD.tuple((uint32_t) AnyPCBlock));
  TerminatorInst *UnexpectedPCJump = UnexpectedPC->getTerminator();
  UnexpectedPCJump->setMetadata("runnable.block.type",
...
//        runnable_assert(getBasicBlockPC(&BB) == 0);
...
//
//    if (Verify.isEnabled())
//      runnable_assert(not verifyModule(TheModule, &dbgs()));
//
//    runnable_log(JTCountLog, "Harvesting: SROA, ConstProp, EarlyCSE and SET");
//
...
//  }
 ...
 // setCFGForm(CFGForm::RecoveredOnlyCFG);
  ...
  if (empty()) {
    runnable_log(JTCountLog, "We're done looking for jump targets");
  }
}
  	...
  	for(auto last : vDefUse){
	    ...
	    case llvm::Instruction::Mul:			  
            ...
            default:
              ...
              runnable_abort("Unkonw instruction!");
            break;
          }
        }
        if(def){
...
uint32_t JumpTargetManager::handleEntryBlock(llvm::BasicBlock *thisBlock, uint64_t thisAddr, uint64_t start, std::map<std::string, llvm::BasicBlock *> &branchlabeledBasicBlock, std::string path){
  ...
  if(br){
    if(!br->isConditional()){
      ...
      runnable_assert(!branchlabeledBasicBlock.empty());
...
bool JumpTargetManager::haveDef(llvm::Instruction *I, llvm::Value *v){
   ...
   switch(result)
   {
     ...
     break;
     case UnknowResult:
         runnable_abort("Unknow of result!");
     break;
   }
   return false;
}
  ...
  for(; I!=rend; I++){
    if(I->getOpcode() == Instruction::Load){
      ...
      if(dyn_cast<Constant>(v)){
        ...
        if(index == UndefineOP)
            runnable_abort("Unkown register OP!\n");
        break;
      }
    }
  }
...
BasicBlock * JumpTargetManager::getSplitedBlock(llvm::BranchInst *branch){
  runnable_assert(!branch->isConditional());
...
//  runnable_assert(isExecutableAddress(PC));
//  if(PC == thisAddr){
//      for(; I!=endInst; I++){
//        if(I->getOpcode() == Instruction::Call){
//          auto callI = dyn_cast<CallInst>(&*I);
...
void JumpTargetManager::handleIndirectJmp(llvm::BasicBlock *thisBlock, 
  ...
  if(store){
    ...
    errs()<<range<<" <---range\n";
    if(range == 0){
      //runnable_abort("Not implement and 'range == 0'\n");
      for(uint64_t n = 0;;n++){
        auto addrConst = foldSet(legalSet,n);
	if(addrConst==nullptr)
	  break;
...
uint32_t JumpTargetManager::getLegalValueRange(llvm::BasicBlock *thisBlock){
  ...
  runnable_assert(cmp,"That should a cmp instruction!");
...
//  return 0;
  ...
  if(firstConst){
    if(legalSet.front().value.size() == 1){
      ...
      runnable_assert(constant,"That should a constant value!\n");
      auto n = constant->getZExtValue();
      return n;
    }
    else{
      runnable_abort("To do more implement!\n");
      //foldstack();
      //return n;
    }
  }
  return 0;
  //runnable_abort("TODO more implement!\n");
  //firstConst ==  false;
  //foldSet(legalSet);
  //return n;
}
...
void JumpTargetManager::analysisLegalValue(std::vector<llvm::Instruction *> &DataFlow,
  ...
  for(unsigned i = 0; i < DataFlow.size(); i++){
	...
	default:
            errs()<<*DataFlow[i];
	    runnable_abort("Unknow of instruction!");
	break;
    }
  }
}
  ...
  for(auto set : make_range(legalSet.rbegin(),legalSet.rend())){
        ...
        case llvm::Instruction::IntToPtr:
	{
	  ...
	  base = ConstantInt::get(base->getType(),addr);
        ...
        default:
	    errs()<<*set.I[0]<<"\n";
            runnable_abort("Unknow fold instruction!");
        break;	    
    }/// end switch(...
  }/// end for(auto..
  return base;
...
void JumpTargetManager::handleSelectOperation(llvm::Instruction *current, 
  ...
  runnable_assert(!isCorrelationWithNext(selectI->getFalseValue(), next),"That's wrong!");
...
void JumpTargetManager::handleBinaryOperation(llvm::Instruction *current, 
  ...
  if(first){
    ...
    errs()<<*next<<"\n";
    runnable_abort("Must one Value has correlation!");
  }
  runnable_assert(((dyn_cast<Constant>(firstOp)&&dyn_cast<Constant>(secondOp)) != 1),"That's unnormal Inst!");
}
...
void JumpTargetManager::harvestbranchBasicBlock(uint64_t nextAddr,
  ...
  if(auto branch = dyn_cast<BranchInst>(I)){
    if(branch->isConditional()){
      //outs()<<*I<<"\n";
      runnable_assert(size==branchlabeledBasicBlock.size(),
  ...
  if(!branchJT.empty()){
    //runnable_assert(branchJT.size()==2,"There should have tow jump targets!");
