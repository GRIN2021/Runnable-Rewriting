...
#include "runnable/Support/MemoryAccess.h"
...
public:
  ...
  void setSignedness(BasicBlock *BB, const Value *V, bool IsSigned) {
    auto Index = std::make_pair(BB, V);
    auto MapIt = TheMap.find(Index);
    runnable_assert(MapIt != TheMap.end());
...
BoundedValue OSRA::mergePredicate(OSR &BaseOp, Predicate P, Constant *ConstOp) {
  ...
  default:
    runnable_unreachable("Unexpected comparison operator");
  }
}
...
void OSRA::handleComparison(Instruction *I) {
  ...
  if (P == CmpInst::ICMP_EQ || P == CmpInst::ICMP_NE) {
    runnable_assert(Constraints.count(nullptr) == 0);
  ...
  for (auto &LHSPair : LHS.Constants) {
    for (auto &RHSPair : RHS.Constants) {
      ...
      if (LHSPair.second != nullptr) {
        ...
        runnable_assert(RHSPair.second != nullptr);
...
void OSRA::handleBranch(Instruction *I) {
  ...
  if (FilteredCFG.hasNode(Source)) {
    ...
    runnable_assert(Successors.size() <= 2);
  ...
  while (!ConstraintsWL.empty()) {
    ...
    runnable_assert(BlockBlackList.find(Entry.Target) == BlockBlackList.end());
...
void OSRAPass::releaseMemory() {
  runnable_log(ReleaseLog, "OSRAPass is releasing memory");
  freeContainer(OSRs);
  if (BVs) {
    delete BVs;
    BVs = nullptr;
...
OSRAPass::identifyOperands(std::map<const Value *, const OSR> &OSRs,
                           const Instruction *I,
                           const DataLayout &DL) {
  runnable_assert(I->getNumOperands() == 2);
  ...
  if (ReachersIt != LoadReachers.end()) {
    ...
    if (ReacherIt != Reachers.end()) {
      ...
      if (ReacherIt->second == NewOSR
        ...
        runnable_assert(!(Reachers.size() > 1 && ReacherValue == Load
...
void OSRA::mergeLoadReacher(LoadInst *Load) {
  ...
  runnable_assert(Reachers.size() > 0);
...
BoundedValue OSRA::pathSensitiveMerge(LoadInst *Reached) {
  ...
  runnable_log(Log, "Performing PSM for " << getName(Reached));
  ...
  while (!Stack.empty()) {
    ...
    runnable_log(Log, Indent << "Exploring " << getName(Pred));
    ...
    for (Reacher &R : Reachers) {
      ...
      if (R.isLTR(Pred)) {
        ...
        runnable_log(Log, Indent << "  Deactivating reacher " << ReacherIndex);
    ...
    if (AlreadyInStack)
      runnable_log(Log, Indent << "    It's already on the stack");

    // Check we're not exceeding the maximum allowed depth
    Proceed &= Height < MaxDepth;
    if (not(Height < MaxDepth))
      runnable_log(Log, Indent << "    We exceeded the maximum depth");
    ...
    if (not(NewPredIt != filtered_pred_end(Pred)))
      runnable_log(Log, Indent << "    No predecessors");
  ...
  runnable_log(Log, "FinalBV: " << FinalBV.describe());

  runnable_assert(not FinalBV.isUninitialized());
  return FinalBV;
}
...
bool OSRAPass::runOnModule(Module &M) {
  runnable_log(PassesLog, "Starting OSRAPass");
  ...
  runnable_log(PassesLog, "Ending OSRAPass");
  return false;
}
...
BoundedValue &BVMap::summarize(BasicBlock *Target, MapValue *BVOVector) {
  ...
  runnable_assert(PredecessorsCount >= BVOVector->Components.size());
...
public:
  ...
  getBV(const BoundedValue &Base, boost::icl::interval_set<T> Intervals) {
    ...
    if (Intervals.iterative_size() == 0) {
      ...
      runnable_assert(Result.Bounds.size() == 0 || Result.Bounds.size() == 1);
      Result.Bounds.clear();
      for (auto Interval : Intervals) {
        const auto static_closed = interval_bounds::static_closed;
        runnable_assert(Interval.bounds().bits() == static_closed);
        ...
        for (auto &Pair : Result.Bounds) {
          if (&Pair != &Result.Bounds.back()) {
            runnable_assert(Pair != Result.Bounds.back());
          }
        }
      }
      runnable_assert(Result.Bounds.size() != 0);
    }
...
bool BoundedValue::slowCompare(const BoundedValue &Other) const {
  runnable_assert(hasSignedness() && Other.hasSignedness());
  runnable_assert(Sign == Other.Sign);
