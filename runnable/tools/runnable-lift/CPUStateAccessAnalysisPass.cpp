  ...
  for (const Function &F : M) {
    for (const Use &U : F.uses()) {
      ...
      if (const auto *TheCall = dyn_cast<const CallInst>(TheUser)) {
        ...
        runnable_assert(CExpr->getOpcode() == Instruction::BitCast);
          ...
          for (const ConstantExpr *BitCast : CurBitCasts) {
            for (const User *BitCastUser : BitCast->users()) {
              ...
              if (TheCall) {
                ...
                runnable_assert(NewCExpr->getOpcode() == Instruction::BitCast);
...
forwardTaintAnalysis(GlobalVariable *CPUStatePtr,
  ...
  runnable_assert(CPUStatePtr != nullptr);
  runnable_assert(CPUStatePtr->getType()->isPointerTy());
  ...
  for (const User *U : CPUStatePtr->users()) {
    ...
    runnable_assert(Load->getPointerOperand() == CPUStatePtr);
    ...
    while (not ToTaintWorkList.empty()) {
      ...
      runnable_assert(TheUse != nullptr);
      ...
      case Instruction::Load: {
        ...
        runnable_assert(OperandNo == LoadInst::getPointerOperandIndex());
      ...
      case Instruction::Add: {
        ...
        runnable_assert(OpCode != Instruction::GetElementPtr
      ...
      case Instruction::Call: {
        ...
        if (Callee->getIntrinsicID() == Intrinsic::memcpy) {
          ...
          runnable_assert(OpNo == 0 or OpNo == 1);
        ...
        runnable_assert(ReachableFunctions.count(Callee) != 0);
        ...
        runnable_assert(FormalArgument != nullptr);
      ...
      case Instruction::Ret: {
        ...
        // (propagation from the callee to all call sites)
        TaintLog << "RET" << DoLog;
        runnable_assert(not CallSiteInfos.empty());
      ...
      while (not ToTaintWorkList.empty() and UnexploredUse == nullptr) {
        ...
        if (Arg and PoppedHasAllExploredSources) {
          ...
          const CallSiteInfo &CSInfo = CallSiteInfos.top();
          runnable_assert(CSInfo.Arg == Arg);

          unsigned ArgNo = Arg->getArgNo();
          runnable_assert(CSInfo.ArgNo == ArgNo);
...
public:
  ...
  explicit WorkItem(Instruction *I) :
    ...
    runnable_assert(not Sources.empty());
  }
  ...
  explicit WorkItem(Argument *A,
    ...
    for (const Use &FUse : F->uses()) {
      ...
      if (const auto *FCall = dyn_cast<const CallInst>(FUser)) {
        ...
        runnable_assert(OpCode == Instruction::BitCast);
  ...
  explicit WorkItem(CallInst *C, bool IsLoad) :
    ...
    runnable_assert(F != nullptr); // Assume no indirect calls
    if (F->getIntrinsicID() == Intrinsic::memcpy) {
      ...
      for (const BasicBlock &BB : *F) {
        ...
        if (I and isa<ReturnInst>(I) and I->getNumOperands() != 0) {
          runnable_assert(I->getNumOperands() == 1);
...
public:
  ...
  void fold(const WorkItem &Item, ValueCallSiteOffsetMap &OffsetMap) {
    ...
    for (const Use *U : Item.sources()) {
      ...
      runnable_assert(not CallSiteOffsets.empty());
    ...
    runnable_assert(NumSrcs == SrcCallSiteOffsetsPtrs.size());
    runnable_assert(NumSrcs == NonRootOffsetsPtrs.size());
    ...
    for (const auto &CSOffsets : SrcCallSiteOffsetsPtrs) {
      ...
      runnable_assert(found_nullptr or found_all_calls);
    }
    ...
    for (CallInst *C : CallSites) {
      ...
      runnable_assert(NumSrcs < (8ULL * sizeof(uint64_t)));
      ...
      for (uint64_t i = 0; i < combinations; ++i) {
        ...
        std::tie(Valid,
                 ResKind) = T::checkOffsetTupleIsValid(OffsetTuple,
                                                       I,
                                                       UpdatedOffsetTuple);
        runnable_assert(UpdatedOffsetTuple.size() == OffsetTuple.size());
        if (not Valid) {
        ...
        for (WorkItem::size_type SI = 0; SI < NumSrcs; ++SI) {
          ...
          runnable_assert(OffsetSize);
          runnable_assert(CartesianSize <= CartesianSize * OffsetSize);
...
private:
  ...
  checkOffsetTupleIsValid(const SmallVector<const CSVOffsets *, 4> &OffsetTuple,
    ...
    runnable_assert(OffsetTuple.size() == 2);
    auto OpCode = I->getOpcode();
    runnable_assert(OpCode == Instruction::Add or OpCode == Instruction::Sub);
    const auto O0 = OffsetTuple[0], O1 = OffsetTuple[1];
    if (OpCode == Instruction::Add) {
      // Cannot add pointers
      runnable_assert(not(O0->isPtr() and O1->isPtr()));
    } else {
      // Cannot subtract a pointer from something else
      runnable_assert(not O1->isPtr());
    }
...
private:
  ...
  checkOffsetTupleIsValid(const SmallVector<const CSVOffsets *, 4> &OffsetTuple,
    ...
    if (GEP->hasIndices() and PointeeTy->isArrayTy()) {
      ...
      runnable_assert(not(FirstIdxConst != nullptr) or FirstIdxPropagatedConst);

      if (FirstIdxPropagatedConst) {

        bool FirstIdxZero = FirstIdxConst->isZero();
        bool FirstIdxPropagatedZero = *OffsetTuple[1]->begin() == 0;
        runnable_assert(not FirstIdxZero or FirstIdxPropagatedZero);
        ...
        if (FirstIdxPropagatedZero) {
          ...
          for (; IdxIt != IdxEnd; ++IdxIt, ++IdxOpNum) {
            ...
            runnable_assert(not IdxCSVOffset->isPtr());
            ...
            if (ElementTy->isAggregateType()) {
              if (ElementTy->isArrayTy()) {
                ...
                if (IdxCSVOffset->isUnknown()) {
                  if (IdxIt + 1 != IdxEnd) {
                    ...
                    runnable_assert(not LastTypeOffsets.empty());
                    CSVOffsets U(CSVOffsets::Kind::Numeric, LastTypeOffsets);
                    UpdatedOffsetTuple[IdxOpNum] = std::move(U);
                    break;
                  }
                  ...
                  // If it's not Unknown we can leave it like it is.
                  runnable_assert(not IdxCSVOffset->empty());
                  ConstIdxList.push_back(*IdxCSVOffset->begin());

                  runnable_assert(IdxCSVOffset->size() != 0);
                  LastTypeOffsets.clear();
              ...
              // I'm done.
              runnable_assert(IdxIt + 1 == IdxEnd);
              if (IdxCSVOffset->isUnknown()) {
                CSVOffsets U(CSVOffsets::Kind::Numeric, LastTypeOffsets);
                UpdatedOffsetTuple[IdxOpNum] = std::move(U);
              } else {
    ...
    for (size_t O = 1; O < NOperands; O++) {
      runnable_assert(not OffsetTuple[O]->isPtr());
...
private:
  ...
  checkOffsetTupleIsValid(const SmallVector<const CSVOffsets *, 4> &OffsetTuple,
    ...
    auto OpCode = I->getOpcode();
    runnable_assert(OpCode == Instruction::Shl or OpCode == Instruction::AShr
    ...
    const auto O0 = OffsetTuple[0], O1 = OffsetTuple[1];
    runnable_assert(not O0->isPtr() and not O1->isPtr());
    if (O0->isUnknown() or O1->isUnknown()) {
...
void CPUSAOA::computeOffsetsFromSources(const WorkItem &Item, bool IsLoad) {
  ...
  if (isa<PHINode>(ItemVal) or isa<Argument>(ItemVal)
    ...
    if (Callee != nullptr and Callee->getIntrinsicID() == Intrinsic::memcpy) {
      ...
      runnable_assert(isa<ConstantInt>(Call->getArgOperand(2)));
    ...
    default:
      runnable_abort(dumpToString(Instr).data());
    }
  } else {
    runnable_abort();
  }
...
void CPUSAOA::computeAggregatedOffsets() {
  ...
  for (std::pair<Value *const, CallSiteOffsetMap> &ACSO : AccessCSOffsets) {
    ...
    runnable_assert(isInstr and (isCorrectAccessType or isCallToBuiltinMemcpy));
    auto *Instr = dyn_cast<Instruction>(I);
    runnable_assert(Tainted.count(Instr) != 0);
...
static void fixEnv2EnvMemCopies(const Module &M,
  ...
  for (auto &InstCSOffset : CSVLoadOffsetMap) {
    ...
    if (It != CSVStoreOffsetMap.end()) {
      ...
      runnable_assert(Memcpy != nullptr
...
CPUStateAccessAnalysis::setupOutEnvAccess(Instruction *AccessToFix) {
  ...
  runnable_assert(InstrIt != AccessToFixBB->end());
...
void CPUStateAccessAnalysis::correctCPUStateAccesses() {
  ...
  for (std::pair<Instruction *const, CSVOffsets> &IOff : CSVAccessOffsetMap) {
    ...
    FixAccessLog << "Fixing access: " << I
                 ...
                 << DoLog;
    runnable_assert(CSVOffsets::isPtr(OKind));
    ...
    case CSVOffsets::Kind::KnownInPtr: {
      ...
      runnable_log(FixAccessLog, "Before: " << dumpToString(F));
      ...
      runnable_assert(AccessToFix != nullptr);
      LLVMContext &Context = M.getContext();
      ...
      // Create the default BB for the switch, calling runnable_abort()
      ...
      if (OffsetValue == nullptr)
        OffsetValue = buildEnvOffsetValue(Builder,
                                          ...
                                          Address,
                                          Int64Ty);
      runnable_assert(OffsetValue != nullptr);
      ...
      if (CSVOffsets::isUnknownInPtr(OKind)) {
        ...
        runnable_assert(SwitchOffset->getNumCases() > 0);
        break; // out from the switch, to the verify and cleanup code
      }

      runnable_assert(Offsets.size() > 0);
      SwitchInst *SwitchOffset = Builder.CreateSwitch(OffsetValue,
                                                      Default,
                                                      Offsets.size());
    ...
    runnable_log(FixAccessLog, "After: " << dumpToString(F));
    if (I != AccessToFix) {
      if (FixAccessLog.isEnabled()) {
...
bool CPUStateAccessAnalysis::run() {
  ...
  QuickMetadata QMD(Context);
  if (Found) {
    auto LoadMDKind = Context.getMDKindID("runnable.csvaccess.offsets."
                                          "load");
    addAccessMetadata(CallSiteLoadOffset, Variables, QMD, LoadMDKind);
    auto StoreMDKind = Context.getMDKindID("runnable.csvaccess.offsets."
                                           "store");
  ...
  runnable_assert(not verifyModule(M, &dbgs()));
