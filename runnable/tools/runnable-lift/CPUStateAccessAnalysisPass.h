...
public:
  ...
  CSVOffsets(Kind K) : OffsetKind(K), Offsets() {
    // Useful for debug runnable_assert(isUnknown(K) or isUnknownInPtr(K));
  }
  CSVOffsets(Kind K, int64_t O) : OffsetKind(K), Offsets({ O }) {
    // Useful for debug runnable_assert(not isUnknown(K) and not
    // isUnknownInPtr(K));
  }
  ...
  void combine(const CSVOffsets &other) {
...

    {
      ...
      if (KIP0 or K1 == Kind::KnownInPtr) {
        runnable_assert(isNumeric(K0) or isNumeric(K1) or isUnknown(K0)
                     or isUnknown(K1));
        OffsetKind = Kind::OutAndKnownInPtr;
        if (not KIP0)
    ...
    runnable_assert((isNumeric(K0) and isUnknown(K1))
