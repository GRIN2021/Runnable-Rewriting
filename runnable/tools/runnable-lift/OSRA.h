...
public:
  ...
  public:
    ...
    bool isSigned() const {
      runnable_assert(Sign != UnknownSignedness && Sign != AnySignedness
                   && !Bottom);
    ...
    actualBoundaries(llvm::Type *Int64) const {
      runnable_assert(!(Negated && isConstant()));
      ...
      runnable_abort("The BV is unlimited");
    }
  ...
  private:
    uint64_t lowerExtreme() const {
      ...
      case InconsistentSignedness:
        return std::numeric_limits<uint64_t>::min();
      default:
        runnable_unreachable("Unexpected signedness");
      }
    }
  ...
  public:
    ...
    public:
      ...
      bool operator==(BoundsIterator &Other) const {
        runnable_assert(TheOSR.Factor == Other.TheOSR.Factor);
