...
BinaryFile::BinaryFile(std::string FilePath, uint64_t BaseAddress) :
  ...
  if (TheArchitecture.pointerSize() == 32) {
    ...
    runnable_assert("Unexpect address size");
  }
...
public:
  ...
  uint64_t addressAtOffset(uint64_t Offset) {
    runnable_assert(HasAddress and HasSize);
    runnable_assert(Offset <= Size);
  ...
  ArrayRef<T> extractAs(const std::vector<SegmentInfo> &Segments) const {
    ...
    runnable_assert(Data.size() % TypeSize == 0);
  ...
  extractData(const std::vector<SegmentInfo> &Segments) const {
    ...
    for (const SegmentInfo &Segment : Segments) {
      if (Segment.contains(Address)) {
        ...
        if (HasSize) {
          runnable_assert(AvailableSize >= Size);
          TheSize = Size;
        }
...
void BinaryFile::parseELF(object::ObjectFile *TheBinary, uint64_t BaseAddress) {
    ...
    for (auto &Section : *Sections) {
      ...
      if (NameOrErr) {
        ...
        if (Name == ".symtab") {
          ...
          runnable_assert(SymtabShdr == nullptr, "Duplicate .symtab");
          SymtabShdr = &Section;
        } else if (Name == ".eh_frame") {
          runnable_assert(not EHFrameAddress, "Duplicate .eh_frame");
	  ...
	  ehframeEndAddr = relocate(static_cast<uint64_t>(Section.sh_addr + Section.sh_size));
          ...
          runnable_assert(not DynamicAddress, "Duplicate .dynamic");
  ...
  for (Elf_Phdr &ProgramHeader : *ProgHeaders) {
    ...
    case ELF::PT_GNU_EH_FRAME:
      runnable_assert(!EHFrameHdrAddress);
      EHFrameHdrAddress = relocate(ProgramHeader.p_vaddr);
      break;

    case ELF::PT_DYNAMIC:
      runnable_assert(DynamicPhdr == nullptr, "Duplicate .dynamic program header");
      DynamicPhdr = &ProgramHeader;
      runnable_assert(((not DynamicAddress)
  ...
  runnable_assert((DynamicPhdr != nullptr) == (DynamicAddress.hasValue()));
  ...
  if (EHFrameHdrAddress) {
    ...
    if (EHFrameAddress) {
      runnable_assert(*EHFrameAddress == Address);
    }
  ...
  if (DynamicPhdr != nullptr) {
    ...
    for (Elf_Dyn &DynamicTag : *DynamicEntries) {
      ...
      case ELF::DT_REL:
      case ELF::DT_RELA:
        runnable_assert(TheTag == (HasAddend ? ELF::DT_RELA : ELF::DT_REL));
        ReldynPortion.setAddress(relocate(DynamicTag.getPtr()));
        break;

      case ELF::DT_RELSZ:
      case ELF::DT_RELASZ:
        runnable_assert(TheTag == (HasAddend ? ELF::DT_RELASZ : ELF::DT_RELSZ));
    ...
    if (NeededLibraryNames.size() > 0)
      runnable_assert(DynstrPortion.isAvailable());
...
uint64_t BinaryFile::symbolsCount(const FilePortion &Relocations) {
  ...
  runnable_assert(Relocations.isExact());
...
BinaryFile::readRawValue(uint64_t Address, unsigned Size, Endianess E) const {
  ...
  for (auto &Segment : segments()) {
    ...
    if (Segment.contains(Address, Size) && Segment.IsReadable) {
      ...
      default:
        runnable_abort("Unexpected read size");
      }
    }
  }
...
Label BinaryFile::parseRelocation(unsigned char RelocationType,
  ...
  case RD::Invalid:
  default:
    runnable_abort("Invalid relocation type");
    break;
  }
}
...
void BinaryFile::registerRelocations(Elf_Rel_Array<T, HasAddend> Relocations,
  ...
  for (Elf_Rel Relocation : Relocations) {
    ...
    if (Dynsym.isAvailable() and Dynstr.isAvailable()) {
      ...
      runnable_check(SymbolIndex < Symbols.size());
...
public:
  ...
  template<typename T>
  T readNext() {
    runnable_assert(Cursor + sizeof(T) <= End);
  ...
  Pointer readPointer(unsigned Encoding, uint64_t Base = 0) {
    runnable_assert((Encoding & ~(0x70 | 0x0F | dwarf::DW_EH_PE_indirect)) == 0);
    ...
    case dwarf::DW_EH_PE_sdata8:
      return readPointerInternal(readNext<int64_t>(), Encoding, Base);
    default:
      runnable_unreachable("Unknown Encoding");
    }
  }

  void moveTo(uint64_t Offset) {
    const uint8_t *NewCursor = Start + Offset;
    runnable_assert(NewCursor >= Cursor && NewCursor <= End);
    Cursor = NewCursor;
  }
...
private:
  ...
  Pointer readPointerInternal(T Value, unsigned Encoding, uint64_t Base) {
    ...
    if (Value != 0) {
      ...
      runnable_assert(EncodingRelative == 0 || EncodingRelative == 0x10);
...
BinaryFile::ehFrameFromEhFrameHdr(uint64_t EHFrameHdrAddress) {
  ...
  runnable_assert(R, ".eh_frame_hdr section not available in any segment");
  ...
  runnable_assert(VersionNumber == 1);
...
void BinaryFile::parseEHFrame(uint64_t EHFrameAddress,
                              Optional<uint64_t> FDEsCount,
                              Optional<uint64_t> EHFrameSize) {
  runnable_assert(FDEsCount || EHFrameSize);
  ...
  while (!EHFrameReader.eof()
    ...
    if (Length == 0) {
      runnable_assert(EHFrameReader.offset() == EndOffset);
      continue;
    }
    ...
    if (ID == 0) {
      ...
      if (!AugmentationString.empty() && AugmentationString.front() == 'z') {
        ...
        for (unsigned I = 1, e = AugmentationString.size(); I != e; ++I) {
          ...
          switch (Char) {
          case 'e':
            runnable_assert((I + 1) != e && AugmentationString[I + 1] == 'h',
          ...
          case 'L':
            ...
            runnable_assert(!LSDAPointerEncoding, "Duplicate LSDA encoding");
            LSDAPointerEncoding = EHFrameReader.readNextU8();
            break;
          case 'P': {
            runnable_assert(!PersonalityEncoding, "Duplicate personality");
            ...
            runnable_log(EhFrameLog, "Personality function: " << PersonalityPtr);
            // TODO: technically this is not a landing pad
            LandingPads.insert(PersonalityPtr);
            break;
          }
          case 'R':
            runnable_assert(!FDEPointerEncoding, "Duplicate FDE encoding");
            FDEPointerEncoding = EHFrameReader.readNextU8();
            break;
          case 'z':
            runnable_unreachable("'z' must be first in the augmentation string");
          }
        }
      }
      ...
      runnable_assert(CIEIt != CachedCIEs.end(),
      ...
      runnable_assert(CIE.FDEPointerEncoding,
      ...
      runnable_log(EhFrameLog, "PCBegin: " << std::hex << PCBegin);
...
void BinaryFile::parseLSDA(uint64_t FDEStart, uint64_t LSDAAddress) {
  runnable_log(EhFrameLog, "LSDAAddress: " << std::hex << LSDAAddress);

  auto R = getAddressData(LSDAAddress);
  runnable_assert(R, "LSDA not available in any segment");
  ...
  runnable_log(EhFrameLog, "LandingPadBase: " << std::hex << LandingPadBase);
