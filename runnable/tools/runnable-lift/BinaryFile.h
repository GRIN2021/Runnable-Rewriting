...
inline SymbolType::Values fromELF(unsigned char ELFSymbolType) {
  ...
  default:
    return SymbolType::Unknown;   
    //runnable_abort("Unexpected symbol type");
  }
}
...
public:
  ...
  uint64_t value() const {
    runnable_assert(isAbsoluteValue() or isBaseRelativeValue());
    return Value;
  }

  llvm::StringRef symbolName() const {
    runnable_assert(isSymbolRelativeValue() or isSymbol());
    return SymbolName;
  }

  uint64_t offset() const {
    runnable_assert(isSymbolRelativeValue());
    return Value;
  }
...
public:
  ...
  uint64_t getPointer(Pointer Ptr) const {
    ...
    runnable_assert(R, "Pointer not available in any segment");
