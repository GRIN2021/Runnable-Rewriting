...
public:
  ...
  static Interrupt createWithSuccessor(Element Result,
    ...
    runnable_assert(Type == BranchType::FakeFunctionCall
  ...
  static Interrupt createWithSuccessors(Element Result,
                                        BranchType::Values Type,
                                        vector Successors) {
    runnable_assert(Type == BranchType::InstructionLocalCFG
                 || Type == BranchType::FunctionLocalCFG);
    runnable_assert(Successors.size() > 0);
  ...
  static Interrupt create(Element Result, BranchType::Values Type) {
    ...
    runnable_assert(Type == Return || Type == IndirectTailCall
...
public:
  ...
  Element &&extractResult() {
    runnable_assert(Type != BranchType::FunctionSummary
                 and Type != BranchType::UnhandledCall);

    runnable_assert(not ResultExtracted);
    ResultExtracted = true;
    return std::move(Result);
  }

  llvm::BasicBlock *getCallee() const {
    runnable_assert(Type == BranchType::UnhandledCall);
    runnable_assert(RelatedBasicBlocks.size() == 1);
...
public:
  ...
  llvm::Optional<int32_t> frameSizeAt(FunctionCall Location) const {
    ...
    runnable_assert(It != FrameSizeAtCallSite.end(),
