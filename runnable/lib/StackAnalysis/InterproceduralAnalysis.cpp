...
void InterproceduralAnalysis::run(BasicBlock *Entry, ResultsPool &Results) {
  ...
  runnable_assert(InProgress.size() == 0);
  ...
  if (Cached) {
    ...
    if (Type == FunctionType::Regular)
      runnable_assert(Summary.BranchesType.size() != 0);
  ...
  runnable_log(SaInterpLog, "Running interprocedural analysis on " << Entry);
  ...
  do {
    ...
    runnable_assert(Result.requiresInterproceduralHandling());

    switch (Result.type()) {
    case BranchType::FakeFunction:
      runnable_log(SaInterpLog, Current.entry() << " is fake");
    ...
    case BranchType::UnhandledCall: {
      ...
      if (getRecursionRoot(Callee) != nullptr) {
        ...
        runnable_assert(TheCache.get(Callee));
    ...
    case BranchType::FunctionSummary: {
      ...
      runnable_log(SaInterpLog, "We have a summary for " << Current.entry());
      ...
      if (Offending.size() != 0) {
        ...
        for (BasicBlock *Entry : Offending) {
          ...
          runnable_assert(Current.entry() != Entry);
        ...
        runnable_assert(TheCache.get(Current.entry()));
      }

      if (SaLog.isEnabled()) {
        runnable_log(SaLog, "FinalState for " << getName(Current.entry()));
        Summary.dump(getModule(Entry), SaLog);
      }

      if (MustReanalyze) {
        runnable_log(SaInterpLog, "Something has changed, let's reanalyze");
        ...
        case BranchType::NoReturnFunction:
          runnable_log(SaInterpLog, Current.entry() << " doesn't return");
    ...
    case BranchType::Invalid:
      runnable_abort("Unexpected branch type in interprocedural analysis");
    }

  } while (InProgress.size() > 0);

  runnable_assert(Type != FunctionType::Invalid);
...
void ResultsPool::mergeFunction(BasicBlock *Function,
  ...
  for (auto &Slot : Slots) {
    ...
    runnable_assert(Slot.first.addressSpace() == ASID::cpuID()
  ...
  for (auto &P : ToImport) {
    ...
    if (It != CallSites.end()) {
      if (!compareOptional(It->second, P.second)) {
        runnable_abort("This call site has a stack at a different height than"
                    " previously recorded");
      }
    } else {
  ...
  static ClobberedMap run(ResultsPool &This) {
    ...
    do {
      ...
      for (int32_t Clobbered : LastResult.Clobbered[nullptr])
        runnable_assert(IndirectCallClobbered.count(Clobbered) != 0);
...
FunctionsSummary ResultsPool::finalize(Module *M) {
  ...
  for (auto &P : FunctionCallSlots) {
    CallSite Call = P.first;
    runnable_assert(CallSites.count(Call) != 0);
    Instruction *I = Call.callInstruction();
    ...
    for (ASSlot Slot : P.second) {
      ...
      runnable_assert(Slot.addressSpace() == CPU);
