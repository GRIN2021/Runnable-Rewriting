...
void FRA::combine(const FCRA &Other) {
  ...
  runnable_assert(Other.Value == FunctionCallRegisterArgument::Maybe
               || Other.Value == FunctionCallRegisterArgument::Yes);

  runnable_assert(Value == NoOrDead || Value == Maybe || Value == Contradiction
...
void FCRA::combine(const FRA &Other) {
  ...
  runnable_assert(Value == Maybe || Value == Yes);

  runnable_assert(Other.Value == FunctionRegisterArgument::NoOrDead
...
void FunctionReturnValue::combine(const FunctionCallReturnValue &Other) {
  ...
  runnable_assert(Value == Maybe || Value == YesCandidate || Value == Dead
               || Value == Yes || Value == NoOrDead);

  // Other is affected by URVOFC and DRVOFC, so that possible states are Maybe,
  // NoOrDead and Yes
  runnable_assert(Other.Value == FunctionCallReturnValue::Maybe
...
void FunctionCallReturnValue::combine(const FunctionReturnValue &Other) {
  ...
  // *this has seen only URVOF, which can only have Maybe or Yes value
  runnable_assert(Other.Value == FunctionReturnValue::Maybe
               || Other.Value == FunctionReturnValue::YesCandidate);

  // Other is affected by URVOFC and DRVOFC, so that possible states are Maybe,
  // NoOrDead, Yes and Contradiction
  runnable_assert(Value == Maybe || Value == NoOrDead || Value == Yes
               || Value == Contradiction);
...
void FunctionsSummary::dumpInternal(const Module *M,
  ...
  for (User *U : M->getFunction("newpc")->users()) {
    ...
    runnable_assert(Address > 0 && Size > 0);
  ...
  for (auto &P : SortedFunctions) {
    ...
    if (Entry != nullptr) {
      ...
      MDNode *Node = T->getMetadata("runnable.jt.reasons");
    ...
    for (const Pair *P : SortedBasicBlocks) {
      ...
      if (It != Coverage.end()) {
        ...
        runnable_assert(IntervalSet.iterative_size() == 1);
