...
cmp(const DefaultMap<K, V, N> &This, const DefaultMap<K, V, N> &Other) {
  ...
  for (auto &P : This) {
    ...
    ROA((P.second.template cmp<Diff, EarlyExit>(Other.getOrDefault(P.first))),
        { runnable_log(SaDiffLog, P.first); });
  }
...
public:
  ...
  unsigned cmp(const Element &Other, const Module *M = nullptr) const {
    ...
    ROA((registerCmp(RegisterAnalyses, Other.RegisterAnalyses, CPU, M)),
        { runnable_log(SaDiffLog, "RegisterAnalyses"); });
...
public:
  ...
  bool isReturn() const {
    runnable_assert(TheReason == Regular or TheReason == Return);
...
public:
  ...
  void initialize() {
    runnable_assert(FirstRun, "The ABIAnalysis cannot be run twice");
    FirstRun = false;
    Base::initialize();
  }
  ...
  Interrupt<E> transfer(ABIIRBasicBlock *BB) {
    runnable_log(SaABI, "Analyzing " << BB->basicBlock());
...
void FunctionABI::analyze(const ABIFunction &TheFunction) {
  ...
  {
    runnable_log(SaABI, "Running forward function analyses");
