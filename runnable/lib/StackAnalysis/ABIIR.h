...
private:
  ...
  ABIIRInstruction(Opcode O, ASSlot Target) :
    ...
    ABI(),
    WrittenStackSlots() {
    runnable_assert(O == Load || O == Store);
  }
  ...
  ABIIRInstruction(Opcode O, FunctionCall Call) :
    ...
    ABI(),
    WrittenStackSlots() {
    runnable_assert(O == IndirectCall);
  }
...
public:
  ...
  FunctionCall call() const {
    ...
    runnable_assert(Call.callInstruction() != nullptr);
    return Call;
  }
...
public:
  ...
  void clear() {
    runnable_assert(Predecessors.empty());
    Instructions.clear();
    Successors.clear();
    IsReturn = false;
  }
