...
void Analysis::initialize() {
  ...
  runnable_log(SaLog, "Creating Analysis for " << getName(Entry));
...
public:
  ...
  void set(Instruction *I, Value V) {
    runnable_assert(I != nullptr);
    runnable_assert(I->getParent() == BB,
                 "Instruction from an unexpected basic block");
    runnable_assert(InstructionContent.count(I) == 0,
...
Interrupt Analysis::transfer(BasicBlock *BB) {
  ...
  runnable_assert(It != State.end());
  Element Result = It->second.copy();

  runnable_log(SaBBLog, "Analyzing " << getName(BB));
  ...
  for (Instruction &I : *BB) {
    ...
    runnable_log(SaVerboseLog, "NewInstruction: " << getName(&I));
        ...
        if (IsContiguousMask) {
          ...
          if (Pointer != nullptr and GCBI->isSPReg(Pointer)) {
            ...
            if (GCBI->pcRegSize() == 4) {
              ...
              runnable_assert(GCBI->pcRegSize() == 8);
    ...
    case Instruction::Call: {
      ...
      runnable_assert(isCallToHelper(&I) || isMarker(&I)
...
Interrupt Analysis::handleTerminator(TerminatorInst *T,
  ...
  runnable_assert(not isa<UnreachableInst>(T));
...
Interrupt Analysis::handleCall(Instruction *Caller,
  ...
  runnable_assert(not(IsRecursive && IsIndirect));
  ...
  if (not IsIndirect) {
    ...
    if (CacheEntry) {
      ...
      runnable_assert(not CacheMustHit);
  ...
  runnable_assert(not(IsIndirectTailCall and IsKiller));
  if (IsIndirectTailCall) {
    return AI::create(std::move(Result), BT::IndirectTailCall);
  } else if (IsKiller) {
    return AI::create(std::move(Result), BT::Killer);
  } else {
    runnable_assert(ReturnFromCall != nullptr);
...
IFS Analysis::createSummary() {
  ...
  if (AnalyzeABI) {
    ...
    if (SaABI.isEnabled()) {
      runnable_log(SaABI, "Starting analysis of " << Entry);
      TheABIIR.dump(SaABI, M);
      SaABI << DoLog;
    }

    runnable_assert(TheABIIR.verify(), "The ABI IR is invalid");
