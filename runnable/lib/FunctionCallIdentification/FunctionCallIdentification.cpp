...
#include "runnable/FunctionCallIdentification/FunctionCallIdentification.h"
...
bool FunctionCallIdentification::runOnModule(llvm::Module &M) {
  runnable_log(PassesLog, "Starting FunctionCallIdentification");
  ...
  if (FunctionCall->empty()) {
    ...
    runnable_assert(FunctionCall->user_begin() == FunctionCall->user_end());
  }
          ...
          if (auto *Store = dyn_cast<StoreInst>(&I)) {
            ...
            if (GCBI.isPCReg(TargetCSV)) {
              ...
              if (Constant->getLimitedValue() == ReturnPC) {
                ...
                runnable_assert(LinkRegister == nullptr);
                if (TargetCSV != nullptr) {
                  ...
                  runnable_assert(LastStackPointer != nullptr);
                  runnable_assert(skipCasts(LastStackPointer) == Pointer);
  ...
  runnable_log(PassesLog, "Ending FunctionCallIdentification");
