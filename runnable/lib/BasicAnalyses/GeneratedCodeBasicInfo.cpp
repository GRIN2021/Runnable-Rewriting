...
bool GeneratedCodeBasicInfo::runOnModule(llvm::Module &M) {
  ...
  runnable_log(PassesLog, "Starting GeneratedCodeBasicInfo");

  RootFunction = &F;

  const char *MDName = "runnable.input.architecture";
  ...
  for (BasicBlock &BB : F) {
    if (!BB.empty()) {
      ...
      case DispatcherBlock:
        runnable_assert(Dispatcher == nullptr);
        Dispatcher = &BB;
        break;

      case DispatcherFailureBlock:
        runnable_assert(DispatcherFail == nullptr);
        DispatcherFail = &BB;
        break;

      case AnyPCBlock:
        runnable_assert(AnyPC == nullptr);
        AnyPC = &BB;
        break;
  ...
  runnable_assert(Dispatcher != nullptr && AnyPC != nullptr
               && UnexpectedPC != nullptr);

  runnable_log(PassesLog, "Ending GeneratedCodeBasicInfo");
