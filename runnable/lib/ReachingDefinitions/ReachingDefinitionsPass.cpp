...
#include "runnable/ReachingDefinitions/ReachingDefinitionsAnalysisImpl.h"
...
bool ReachingDefinitionsPass::runOnModule(llvm::Module &M) {
  runnable_log(PassesLog, "Starting ReachingDefinitionsPass");
  ...
  runnable_log(PassesLog, "Ending ReachingDefinitionsPass");
...
bool ConditionalReachedLoadsPass::runOnModule(llvm::Module &M) {
  runnable_log(PassesLog, "Starting ConditionalReachedLoadsPass");
  ...
  runnable_log(PassesLog, "Ending ConditionalReachedLoadsPass");
...
bool ConditionNumberingPass::runOnModule(Module &M) {
  ...
  runnable_log(PassesLog, "Starting ConditionNumberingPass");
  ...
  for (auto &P : Conditions) {
    ...
    for (BranchInst *T : Sisters) {
      runnable_assert(T->isConditional());
      ...
      for (BasicBlock *Successor : Successors) {
        runnable_assert(Successor->getSinglePredecessor() == T->getParent());
        ...
        runnable_assert(Successor != nullptr);
  ...
  for (BasicBlock *BB : ToDelete) {
    auto It = BB->begin();
    auto End = BB->end();
    runnable_assert(It != End and isa<BranchInst>(&*It));
    It++;
    runnable_assert(It == End);

    BasicBlock *Successor = BB->getSingleSuccessor();
    BasicBlock *Predecessor = BB->getSinglePredecessor();
    runnable_assert(Successor != nullptr and Predecessor != nullptr);

    BB->replaceAllUsesWith(Successor);
    BB->eraseFromParent();
  }

  runnable_log(PassesLog, "Ending ConditionNumberingPass");
