...
#include "runnable/FunctionIsolation/IsolateFunctions.h"
#include "runnable/Runtime/commonconstants.h"
#include "runnable/StackAnalysis/FunctionsSummary.h"
...
void IFI::throwException(Reason Code, BasicBlock *BB, uint64_t AdditionalPC) {
  runnable_assert(PC != nullptr);
  runnable_assert(RaiseException != nullptr);
  runnable_assert(DebugException != nullptr);
...
bool IFI::replaceFunctionCall(BasicBlock *NewBB,
  ...
  if (IsIndirect) {
    ...
    if (Node == nullptr) {
      ...
      runnable_assert(not isTerminatorWithInvalidTarget(T, RootToIsolated));
  ...
  runnable_assert(TargetFunction != nullptr);
    ...
    while (!UserQueue.empty()) {
      ...
      for (Use &CurrentUse : CurrentUser->operands()) {
        ...
        if (ReplacementIt != RootToIsolated.end()) {
          ...
          runnable_assert(RootToIsolated.count(CurrentOperand) != 0);
...
void IFI::run() {
  ...
  runnable_assert(CallMarker != nullptr);
  ...
  for (BasicBlock &BB : *RootFunction) {
    ...
    if (MDNode *Node = Terminator->getMetadata("func.entry")) {
      ...
      runnable_assert(Functions.count(FunctionNameMD) == 0);
  ...
  for (BasicBlock &BB : *RootFunction) {
    ...
    if (MDNode *Node = Terminator->getMetadata("func.member.of")) {
      ...
      for (const MDOperand &Op : Tuple->operands()) {
        ...
        runnable_assert(ParentFunction != nullptr);
  ...
  for (auto &Pair : Functions) {
    ...
    for (BasicBlock &NewBB : *AnalyzedFunction) {
      ...
      for (BasicBlock *Successor : Terminator->successors()) {
        ...
        // We add a successor if it is not a runnable block type and it is present
      ...
      runnable_assert(NewBB.getTerminator() == nullptr);
    ...
    for (auto &P : Descriptor.Members) {
      ...
      runnable_assert(FakeCall != nullptr);
      ...
      runnable_assert(isa<UnreachableInst>(T));
      T->eraseFromParent();
    ...
    runnable_assert(not AnalyzedFunction->isDeclaration());
    ...
    for (BasicBlock &BB : *AnalyzedFunction) {
      ...
      if (&BB != UnexpectedPC && &BB != AnyPC) {
        ...
        runnable_assert(isa<SwitchInst>(I) || isa<UnreachableInst>(I));
        I.eraseFromParent();
      }
    }
    ...
    for (Instruction *OldAlloca : AllocasToClone) {
      ...
      runnable_assert(NewAlloca->getParent() == Dummy);
  ...
  for (BasicBlock &BB : *Root) {
    if (BB.getName() == "unexpectedpc")
      runnable_assert(GCBI.getType(&BB) == UnexpectedPCBlock);
    if (GCBI.getType(&BB) == UnexpectedPCBlock) {
      UnexpectedPC = &BB;
      break;
    }
  }
  runnable_assert(UnexpectedPC != nullptr);
  ...
  // by runnable a call to the newly created corresponding LLVM function
  for (BasicBlock &BB : *Root) {
    runnable_assert(!BB.empty());
  ...
  runnable_assert(verifyModule(*TheModule, &Stream) == false);
}
