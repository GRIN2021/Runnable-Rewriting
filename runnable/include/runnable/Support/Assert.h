#ifndef RUNNABLE_ASSERT_H
#define RUNNABLE_ASSERT_H
...
noret void runnable_assert_fail(const char *AssertionBody,
                             const char *Message,
                             const char *File,
                             unsigned Line);
noret void runnable_check_fail(const char *CheckBody,
                            const char *Message,
                            const char *File,
                            unsigned Line);
noret void runnable_do_abort(const char *Message, const char *File, unsigned Line);
...
/// situation.
#define runnable_abort_impl(message)                \
  do {                                           \
    runnable_do_abort(message, __FILE__, __LINE__); \
  } while (0)
...
/// Use this macro to ensure program termination in case of an unexpected
/// situation.
#define runnable_check_impl(what, message)                     \
  do {                                                      \
    bool Condition = boolcast(what);                        \
    if (!Condition) {                                       \
      runnable_check_fail(#what, message, __FILE__, __LINE__); \
    }                                                       \
...
#define runnable_unreachable_impl(message) runnable_abort(message)
...
/// certain situation will never happen at run-time, which might open up to new
/// optimization opportunities.
#define runnable_assert_impl(what, message)                     \
  do {                                                       \
    bool Condition = boolcast(what);                         \
    if (!Condition) {                                        \
      runnable_assert_fail(#what, message, __FILE__, __LINE__); \
    }                                                        \
    silent_assume(Condition);                                \
  } while (0)
...
#define runnable_unreachable_impl(message) __builtin_unreachable()
...
#define RUNNABLE_ABORT_0(...) runnable_abort_impl(__VA_ARGS__)
#define RUNNABLE_ABORT_1(...) runnable_abort_impl(NULL)
#define runnable_abort(...) \
  CONCAT2(RUNNABLE_ABORT_, IS_EMPTY(__VA_ARGS__))(__VA_ARGS__)

#define RUNNABLE_UNREACHABLE_0(...) runnable_unreachable_impl(__VA_ARGS__)
#define RUNNABLE_UNREACHABLE_1(...) runnable_unreachable_impl(NULL)
#define runnable_unreachable(...) \
  CONCAT2(RUNNABLE_UNREACHABLE_, IS_EMPTY(__VA_ARGS__))(__VA_ARGS__)

#define MACRO_OVERLOAD_1_OR_2(_1, _2, NAME, ...) NAME

#define runnable_assert_impl_nomsg(what) runnable_assert_impl(what, NULL)
#define runnable_assert(...)                        \
  MACRO_OVERLOAD_1_OR_2(__VA_ARGS__,             \
                        runnable_assert_impl,       \
                        runnable_assert_impl_nomsg) \
  (__VA_ARGS__)

#define runnable_check_impl_nomsg(what) runnable_check_impl(what, NULL)
#define runnable_check(...)                                                       \
  MACRO_OVERLOAD_1_OR_2(__VA_ARGS__, runnable_check_impl, runnable_check_impl_nomsg) \
  (__VA_ARGS__)
