...
public:
  ...
  void insert(Iterated Entry) {
    ...
    runnable_assert(It != PostOrderListIndex.end());
  ...
  Iterated head() const {
    runnable_assert(Next != InvalidIndex);
    return PostOrderList[Next].entry();
  }

  Iterated pop() {
    runnable_assert(not empty());
    runnable_assert(verify());
  ...
  Interrupt run() {
    ...
    while (not WorkList.empty()) {
      ...
      if (Result.isReturn()) {
        ...
        runnable_assert(SuccessorsCount == 0);
        ...
        if (DynamicGraph) {
          ...
          if (Successors->size() != 0)
            runnable_assert(NewSuccessors == *Successors);
    ...
    if (DynamicGraph)
      runnable_assert(FirstFinalResult == (FinalStates.size() == 0));
    else
      runnable_assert(FinalStates.size() == 0 and SuccessorsMap.size() == 0);

    // The work list is empty
    runnable_assert(ToVisit.empty());
    if (FirstFinalResult) {
      ...
      if (DynamicGraph) {
        ...
        while (not ReachableLabels.empty()) {
          ...
          runnable_assert(It != SuccessorsMap.end());
        ...
        runnable_assert(Reachable.size() == State.size());
