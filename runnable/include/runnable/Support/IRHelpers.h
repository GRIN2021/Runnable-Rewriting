...
inline void purgeBranch(llvm::BasicBlock::iterator I) {
  ...
  runnable_assert(DeadBranch != nullptr && ++I == DeadBranch->getParent()->end());
...
inline std::tuple<T...> operandsByType(llvm::User *V) {
  ...
  runnable_assert(OpCount == sizeof...(T));
