...
inline llvm::StringRef getName(Values Reason) {
  ...
  runnable_abort("Unexpected reason");
}
...
inline Values fromName(llvm::StringRef Name) {
  ...
  if (Name == "KillerSyscall")
    ...
    runnable_abort("Unexpected name");
}
...
public:
  ...
  bool isJumpOutSupported() const {
    ...
    runnable_assert(IsSupported == (ReadRegisterAsm.size() != 0)
...
inline T *notNull(T *Pointer) {
  runnable_assert(Pointer != nullptr);
  return Pointer;
}
