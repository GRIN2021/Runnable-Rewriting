...
private:
  ...
  struct LargeStorage {
    ...
    uintptr_t &at(size_t Index) {
      runnable_assert(Index < wordCount());
    ...
    void zero(size_t From, size_t Count) {
      runnable_assert(From + Count <= wordCount());
    ...
    LargeStorage &operator=(const LargeStorage &Other) {
      runnable_assert(Capacity >= Other.Capacity);
...
public:
  ...
  void set(unsigned Index) {
    runnable_assert(Index < IntMax);

    if (Index >= capacity())
      alloc(Index + 1);

    if (isSmall()) {
      runnable_assert(Index < MaxSmallSize);
  ...
  void zero(size_t From, size_t Count) {
    ...
    if (isSmall()) {
      ...
      if (HeadComplement != 0) {
        ...
        From += Head;
        Count -= Head;
        runnable_assert(From % BitsPerPointer == 0);
      }
      ...
      if (Tail != 0) {
        ...
        Count -= Tail;
        runnable_assert(Count % BitsPerPointer == 0);
      }
  ...
  LazySmallBitVector &operator^=(const LazySmallBitVector &Other) {
    ...
    runnable_assert(!(isSmall() && !Other.isSmall()));
  ...
  LazySmallBitVector &operator>>=(unsigned Amount) {
    runnable_assert(Amount <= capacity());
...
private:
  ...
  void alloc(size_t NewSize) {
    runnable_assert(NewSize > capacity());
...
inline void LazySmallBitVectorIterator<LSBV>::increment() {
  runnable_assert(BitVector != nullptr);
  runnable_assert(NextBitIndex == 0 || (*BitVector)[NextBitIndex - 1] == true);
