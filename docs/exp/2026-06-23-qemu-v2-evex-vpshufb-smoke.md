# QEMU V2 EVEX VPSHUFB Smoke Patch

Date: 2026-06-23
Branch: `codex/qemu-upgrade-v2`
Script: `runnable/scripts/qemu_v2_evex_vpshufb_smoke_patch.sh`
Scratch root used for fresh validation: `/tmp/rr-qemu-v2-evex-vpshufb-smoke-final2`
...
The aggregate probe now advances past `vpshufb` and still fails at:
...
- `rr_try_evex_vpshufb_smoke(...)`, the exact-byte dispatcher.
...
bash runnable/scripts/qemu_v2_evex_vpshufb_smoke_patch.sh \
  --fresh \
  --scratch-root /tmp/rr-qemu-v2-evex-vpshufb-smoke-final2 \
...
aggregate_result:          EXPECTED_FAIL_AFTER_VPSHUFB
aggregate_run_rc:          132
aggregate_exception_hits:  3
aggregate_vpshufb_hits:    4
aggregate_vpaddd_rip_hits: 1
aggregate_next:            401026: 62 f1 65 48 fe e2 vpaddd zmm4,zmm3,zmm2

failures:                  0
```

Observed TCG for the `vpshufb` smoke path includes four lane helper calls:
...
```text
401026: 62 f1 65 48 fe e2    vpaddd zmm4,zmm3,zmm2
```

## Limitations

- Exact-byte only; no general EVEX opcode or operand decode.
- Only `vpshufb zmm3,zmm2,zmm2` is covered.
